...
{
    dies_ok { $repository->remove_distribution('NOBODY/X-1.0.tar.gz') }
        'trying to remove a non-existing distribution dies';

    $repository->remove_distribution('SMART/Some-Package-1.0.tar.gz');
    ...
    is_deeply $repository->revisions->revision_info,
        [
            ...
            { revision => 2, operation => '+', author => 'CLEVER', file => 'Another-Module-2.07.tar.gz' },
            { revision => 3, operation => '+', author => 'SMART',  file => 'Some-Package-1.0.tar.gz' },
            { revision => 4, operation => '-', author => 'SMART',  file => 'Some-Package-1.0.tar.gz' },
        ],
        'revision_info reflects removal';

    is join('|', sort keys %{$repository->packages->packages_for}),
        'C/CL/CLEVER/Another-Module-2.07.tar.gz',
        'one packages registered after removal';
}
